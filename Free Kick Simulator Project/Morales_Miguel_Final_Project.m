...
t_step= 0.01; %time step between each calculation (s)
...
w = rot*pi*2; %angular velocity on ball (rad/s)
s0= 4e-4; %average drag coeff
a_y = -9.81; %initial acceleration in the y direction (m/s^2)
a_x = 0; %initial acceleration in the x direction (m/s^2)
a_z = 0; %initial acceleration in the z direction (m/s^2)
% v_0 = 40; %initial magnitude of the velocity (m/s)
v_0x = v_0*cos(theta_rad); %initial velocity in the x direction (m/s)
v_0y = v_0*sin(theta_rad); %initial velocity in the y direction (m/s)
v_0z = v_0x*tan(phi_rad); %initial velocity in the z direction (m/s)
d_0y = 0; %initial displacement in the y direction (m)
d_0x = 0; %initial displacement in the x direction (m)
d_0z = 11; %initial displacement in the z direction (m)
r = 0.109; %radius of the soccer ball (m)
p =1.6; %average density of the soccer ball (kg/m^3)
C = 0.15; %drag coefficient
A = 3.14159*(r^2); %silhouette area of soccer ball
D = p*C*A*0.5; %constant
m = 0.5; %mass of soccer ball (kg)
react = 0.20; %reaction time of the goalkeeper (s)
react_step= 0.20/t_step; % num step of reaction time
...
disp(['The final velocity is ', num2str(v_final),' m/s. The final position at time ',num2str(t_vals(final)), ' s is [', num2str(d_x(final)),',',num2str(d_z(final)),',', num2str(d_y(final)),'] m.'])